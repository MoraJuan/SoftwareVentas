...
class PageProductForm(ft.View):
    ...
    def add_stock(self, e):
        ...
        if not self.selected_product:
            show_error_message(
                self.page, "Debe seleccionar un producto primero")
            return
        ...
        try:
            ...
            if adjustment_amount <= 0:
                show_error_message(
                    self.page, "La cantidad a añadir debe ser mayor a cero")
                return
            ...
            self.selected_product = self.product_service.get_product_by_id(
                self.selected_product.id)

        except ValueError:
            show_error_message(
                self.page, "Por favor ingrese un valor numérico válido")
        except Exception as e:
    ...
    def remove_stock(self, e):
        ...
        try:
            ...
            if adjustment_amount <= 0:
                show_error_message(
                    self.page, "La cantidad a reducir debe ser mayor a cero")
                return

            if adjustment_amount > self.selected_product.stock:
                show_error_message(
                    self.page, "No puede reducir más stock del disponible")
                return
