...
class MakeSaleView(ft.View):
    ...
    def build_ui(self):
        ...
        self.total_text = ft.Text("Total: $0.00", size=16, weight=ft.FontWeight.BOLD)
    ...
    def search_products(self, e):
        try:
            ...
            if name:
                ...
                products = self.product_service.get_products_by_price_range(float(min_price), float(max_price))
            else:
                products = self.product_service.get_all_products()
            
            self.product_dropdown.options = [ft.dropdown.Option(key=product.id, text=product.name) for product in products]
    ...
    def add_product(self, e):
        try:
            ...
            if not product_id or not quantity:
                show_error_message(self.page, "Debe ingresar un producto y una cantidad válida")
                return
            ...
            show_error_message(self.page, f"Error al agregar producto: {str(e)}")
