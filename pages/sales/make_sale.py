...
class MakeSaleView(ft.UserControl):
    ...
    def handle_resize(self, e):
        """Maneja el redimensionamiento sin cerrar la vista"""
        try:
            ...
            logging.info(f"MakeSaleView: Redimensionamiento detectado - ancho: {self.page.width}, es_móvil: {self.is_mobile}")
            
            # Si cambia el tipo de dispositivo, reconstruir la UI
            if old_is_mobile != self.is_mobile or old_is_tablet != self.is_tablet:
                logging.info("MakeSaleView: Cambio de tipo de dispositivo, reconstruyendo UI...")
                self.build_ui()
                self.update()
        except Exception as e:
            logging.error(f"Error en redimensionamiento de MakeSaleView: {str(e)}")
            import traceback
    ...
    def build_ui(self):
        try:
            ...
            # Contenido principal - ajustar para móvil
            if self.is_mobile:
                customer_payment_section = ft.Column([
                    self.customer_section,
                    ft.Container(height=10),
                    self.payment_section
                ], spacing=10)
            else:
                customer_payment_section = ft.Row([
                    self.customer_section,
                    self.payment_section
                ], spacing=20)
    ...
    def return_to_sales(self, e=None):
        """Método para volver a la gestión de ventas"""
        try:
            logging.info("Volviendo a la gestión de ventas...")
            
            # Restaurar el manejador de redimensionamiento original
            if hasattr(self, 'original_resize_handler'):
                self.page.on_resize = self.original_resize_handler
                
            # Informar al padre que debe volver a mostrar la vista de ventas
            ...
            # Volver a la vista anterior usando el padre
            from pages.sales import PageSales
            parent_view = PageSales(self.page, self.session)
            self.controls.append(parent_view)
            self.update()
        except Exception as e:
            logging.error(f"Error al volver a ventas: {str(e)}")
            import traceback
            logging.error(traceback.format_exc())
            show_error_message(self.page, f"Error al volver a ventas: {str(e)}")
