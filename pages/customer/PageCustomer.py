...
class PageCustomer(ft.View):
        ...
        try:
            return ft.DataTable(
                columns=[
                    ...
                    ft.DataColumn(
                        ft.Text("Acciones", color=ft.colors.ON_SURFACE)),
                ],
                rows=[],
    ...
    def filter_customers(self, e):
        ...
        try:
            ...
            show_error_message(
                self.page, f"Error al filtrar clientes: {str(e)}")
    ...
    def export_to_csv(self, e):
        ...
        try:
            ...
            show_success_message(
                self.page, "Clientes exportados exitosamente.")
        except Exception as e:
            logging.error(f"Error al exportar clientes: {str(e)}")
            show_error_message(
                self.page, f"Error al exportar clientes: {str(e)}")
    ...
    def sort_customers_by_column(self):
        ...
        try:
            ...
            if self.sort_column_index == 0:  # ID
                def key_func(c): return c.id
            elif self.sort_column_index == 1:  # Nombre
                def key_func(c): return c.name.lower()
            elif self.sort_column_index == 2:  # Email
                def key_func(c): return (c.email or "").lower()
            else:
                # Por defecto ordenar por nombre
                def key_func(c): return c.name.lower()

            # Ordenar la lista de clientes
            self.filtered_customers.sort(
                key=key_func, reverse=not self.sort_ascending)

        except Exception as e:
            logging.error(f"Error al ordenar clientes: {str(e)}")
            show_error_message(
                self.page, f"Error al ordenar clientes: {str(e)}")
    ...
    def go_to_page(self, page_number):
        ...
        try:
            ...
            show_error_message(
                self.page, f"Error al cambiar de página: {str(e)}")
    ...
    def update_table_with_customers(self, customers):
        ...
        try:
            ...
            for customer in customers:
                ...
                row = ft.DataRow(
                    cells=[
                        ft.DataCell(ft.Text(str(customer.id))),  # ID
                        ft.DataCell(ft.Text(customer.name)),     # Nombre
                        ft.DataCell(ft.Text(customer.email)),    # Email
                        ft.DataCell(ft.Text(customer.phone)),
                        ft.DataCell(ft.Text(customer.address)),
                        ft.DataCell(ft.Row([  # Acciones (puedes agregar botones u otros elementos)
                            ft.IconButton(
                                icon=ft.icons.EDIT, on_click=lambda e, c=customer: self.edit_customer(c)),
                            ft.IconButton(
                                icon=ft.icons.DELETE, on_click=lambda e, c=customer: self.delete_customer(c)),
                        ]),
                        ),
                    ]
                )
            ...
            if not rows:
                if self.search_field.value:
                    show_error_message(
                        self.page, f"No se encontraron clientes que coincidan con '{self.search_field.value}'")
                else:
            ...
            show_error_message(
                self.page, f"Error al actualizar tabla: {str(e)}")
    ...
    def view_customer(self, customer):
        ...
        try:
            ...
            show_error_message(
                self.page, f"Error al ver detalles del cliente: {str(e)}")
        ...
        try:
            ...
            self.page.dialog = ft.AlertDialog(
                ...
                actions=[
                    ...
                    ft.TextButton(
                        "Eliminar", on_click=lambda _: self.confirm_delete_customer(customer)),
                ],
                actions_alignment=ft.MainAxisAlignment.END,
            )
            self.page.dialog.open = True
            ...
            show_error_message(
                self.page, f"Error al eliminar cliente: {str(e)}")
    ...
    def confirm_delete_customer(self, customer):
        ...
        try:
            ...
            show_success_message(
                self.page, f"Cliente {customer.name} eliminado correctamente")
        except Exception as e:
            logging.error(f"Error al eliminar cliente: {str(e)}")
            show_error_message(
                self.page, f"Error al eliminar cliente: {str(e)}")
    ...
    def load_customers(self, e=None):
        ...
        try:
            ...
            show_error_message(
                self.page, f"Error al cargar clientes: {str(e)}")
